...
if ($_POST['btnSMP'])
	...
	if (empty($e_nama))
		...
		if ($s == "baru")
			{
			//insert
			mysqli_query($koneksi, "INSERT INTO `cp_foto_seniman` (`kd`, `nama`, `ket`, `filex`, `postdate`) ".
							"VALUES ('$e_kd', '$e_nama', '$e_ket', '$namabaru', '$today');"); 
		...
		if ($s == "edit")
			{
			mysqli_query($koneksi, "UPDATE cp_foto_seniman SET nama = '$e_nama', ".
							"ket = '$e_ket', ".
							"filex = '$namabaru', ".
							"postdate = '$today' ".
							"WHERE kd = '$e_kd'");
...
if ($_POST['btnHPS'])
	...
	for ($i=1; $i<=$jml;$i++)
		...
		//del
		mysqli_query($koneksi, "DELETE FROM cp_foto_seniman ".
						"WHERE kd = '$kd'");
		}
...
if (($s == "baru") OR ($s == "edit"))
	{
	//edit
	$qx = mysqli_query($koneksi, "SELECT * FROM cp_foto_seniman ".
						"WHERE kd = '$kd'");
	...
	<textarea id="e_nama" name="e_nama" rows="2" cols="80" style="width: 100%" class="btn-warning" required>'.$e_nama.'</textarea>
	...
	<textarea id="e_ket" name="e_ket" rows="5" cols="80" style="width: 100%" class="btn-warning" required>'.$e_ket.'</textarea>
	...
	$(document).ready(function(){
	       ...
	       $('#image-holder').load("<?php echo $sumber;?>/adm/se/i_foto.php?aksi=lihat1&kd=<?php echo $kd;?>");
	    ...
	    $('#upload_image').on('change', function(event){
	     event.preventDefault();
			...
			$('#loading').show();
		     ...
		     $.ajax({
		      url:"i_foto_upload.php?kd=<?php echo $kd;?>",
		      method:"POST",
		      ...
		      success:function(data){
				$('#loading').hide();
		       $('#preview').load("<?php echo $sumber;?>/adm/se/i_foto.php?aksi=lihat&kd=<?php echo $kd;?>");
	...
	<div class="table-responsive">          
		  <table class="table" border="1">
			...
			<td width="150"><strong><font color="'.$warnatext.'">PENGIRIM</font></strong></td>
			</tr>
	...
	if ($count != 0)
		...
		do {
			...
			$nomer = $nomer + 1;
			$e_kd = nosql($data['kd']);
			$e_seniman = balikin($data['seniman_nama']);
			...
			<td>
			<p>
			<img src="'.$sumber.'/filebox/foto/'.$e_kd.'/'.$filex1.'" width="200" height="200">
			</p>
